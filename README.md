...
> **주의: 프롬프트 업데이트됨.** 아래는 `build_user_prompt`에 "발화에서 인자 값을 직접
> 추출하라" 지시문을 추가하기 **이전** 프롬프트로 만든 결과 (지시문 추가 배경/효과는 아래
> "Tier-2 ... 실제 retriever에서 EM이 떨어지는 이유 분석 + 프롬프트 수정" 섹션 참고). Tier-1은
> 인자를 안 쓰므로 이 지시문의 영향이 거의 없을 것으로 예상되지만, 아직 새 프롬프트로
> 재실행해서 확인하진 않음 — 재실행 전까지는 참고용.
...
|---|---|
| 152개 전체 | 64.4% |
| 실제 retriever @k=10 (학습 0) | 51.8% |
| 실제 retriever @k=20 (학습 0) | 58.0% |
| domain 필터링 | 78.6% |
| **실제 retriever @k=5 (1 epoch 학습)** | **74.6%** |
| oracle top-5 (GT 보장, 무작위 distractor) | 96.9% |

152개 전체를 그냥 주는 것보다 확실히 나아졌고(64.4%→74.6%), 미학습 real retriever보다도 훨씬
낫다. 다만 recall@5(95.9%)와 downstream Acc(74.6%) 사이 갭(~21%p)이 여전히 큼 — oracle의 top-5는
...
> **주의: 아래 표는 구버전 프롬프트 기준.** `tier1_oracle.py`의 `build_user_prompt`(tier1/tier2
> 공용)에 "발화에서 인자 값을 직접 추출하라, defaults를 베끼지 마라"는 지시문을 나중에
> 추가했는데(바로 아래 섹션 참고), 아직 이 표 전체를 새 프롬프트로 재실행하진 않아서 숫자가
> 서로 다른 프롬프트 버전 기준으로 섞여 있음. 재실행 전까지는 "실제 retriever @k=5" 행만
> 아래 새 섹션의 최신 수치로 대체해서 봐야 함.

| 세팅 | Acc | EM | F1 |
|---|---|---|---|
| 152개 전체 | 73.4% | 44.8% | 63.2% |
| domain 필터링 | 83.8% | 48.2% | 65.3% |
| 실제 retriever @k=5 (1 epoch 학습, 구버전 프롬프트) | 78.9% | 38.8% | 57.6% |
...
failure 71건 중 대부분이 모델이 "이 중엔 맞는 tool이 없다"며 답변을 거부하는 새 패턴 — 지시문이
모델을 더 신중하게 만들어서 5개 후보 중 확신이 안 서면 거부하는 쪽으로 간 것으로 보임. 그래도
순효과는 뚜렷하게 플러스.

이 지시문은 `build_user_prompt`가 tier1/tier2, 모든 세팅(152개 전체/domain 필터링/oracle
top-5/retriever)에 공용이라 지금 코드 기준으로는 이미 전부 적용된 상태 — 위 두 결과표(Tier-1,
Tier-2 4-세팅 비교)는 이 지시문 추가 **이전** 프롬프트로 만들어진 것이라 전부 재실행 전까지는
참고용으로만 봐야 함 (다음 단계 참고).
...
- [ ] **프롬프트 업데이트("발화에서 인자 추출" 지시문) 반영해서 Tier-1/Tier-2 전체 8개 세팅
      재실행** — 지금은 retriever @k=5(Tier-2)만 새 프롬프트로 확인됨(EM 38.8%→48.7%), 나머지는
      구버전 프롬프트 결과라 재실행 전까지 표 안 섞어서 봐야 함

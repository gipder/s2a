...
def encode_texts(texts: list[str], tokenizer, model, batch_size: int, max_length: int) -> torch.Tensor:
    ...
    with torch.no_grad():
        for i in range(0, len(wrapped), batch_size):
            ...
            vecs.append(torch.nn.functional.normalize(pooled.float(), dim=-1).cpu())

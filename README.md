...
## 핵심 결과: 152개 전체(baseline) 대비 개선

**목표는 논문의 절대 수치(EM 10.1% 등)를 맞추는 게 아니라, 우리 자체 152개 전체 baseline 대비
retriever/필터링이 실제로 개선되는가**임 — 논문과의 절대 비교는 채점 방식이 달라서(아래 Tier-2
섹션 참고) 애초에 신뢰하기 어려움. 이 기준으로 보면 결과가 훨씬 깔끔함(2026-07-23, 현재 프롬프트):
...
하에서도(oracle 아님) 두 tier 다 baseline 대비 뚜렷한 개선. 이게 이 프로젝트의 핵심 주장.

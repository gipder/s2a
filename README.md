...
tool 전체에 최소 9개, 평균 19.2개(2,911개 총합) 확보. 다음 단계는 이 데이터로 STOP 체크포인트에서
이어서(warm-start) contrastive LoRA 학습.
...
- [ ] synthetic 데이터로 STOP 체크포인트(`qwen3-0.6b_depth0.1_seed44_5ep/epoch5`)에서 warm-start
      contrastive LoRA 학습 — `train_retriever.py` 참고해서 이 프로젝트 corpus 형식에 맞게 작성 필요
- [ ] 학습된 retriever로 recall@k 개선해서 STOP 전이(recall@5 55.7%) 대비, 궁극적으로 oracle
      상한선(96.9%)에 근접시키기
